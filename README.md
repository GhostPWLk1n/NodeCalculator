...
## 🚀 Quick Start
git clone [https://github.com/GhostPWLk1n/nodecalculate.git](https://github.com/GhostPWLk1n/NodeCalculator.git)
...
## 📊 Доски (Boards) — визуализация отдельно от графа вычислений

Помимо Листов (Layouts) — обычного холста с графом нод — в приложении есть
второй тип холста: **Доски**. Доска не хранит свой граф вычислений, а
показывает выбранные значения из любых Листов в виде страницы виджетов
(готовится под печать/экспорт в PDF). Управляется отдельным менеджером,
`boardManager.js`, который живёт параллельно `layoutManager.js` и не знает
о структуре графа — только о готовых виджетах.

### Как данные попадают на Доску

```
[любая нода-источник] → [DashboardNode] → [BoardManager] → [Доска]
     (Число/Строка/          "мост" на            хранилище              холст-
      Список/Таблица)         Листе                виджетов              страница
```

1. На Листе ставится нода **"Дашборд"** (`dashboardNode.js`) и подключается
   к источнику данных — вход/выход у неё универсальные (`any`), поэтому она
   пропускает через себя что угодно, не разрывая граф.
2. В боковой панели у ноды "Дашборд" выбирается **целевая Доска**
   (`targetBoardId`) и, при желании, порядок виджета на странице
   (`dashboardOrder`).
3. Совместимость с Доской определяется не типом сокета, а тем, реализует ли
   ПОДКЛЮЧЁННЫЙ ИСТОЧНИК метод `getDashboardWidget()` — необязательный хук
   в духе `getInspectorSchema()`. Сейчас его реализуют `numberNode.js`,
   `stringNode.js`, `listInputNode.js` и `tableNode.js`; любая новая нода
   может добавить его по тому же образцу, и её сразу можно будет вывести на
   Доску без правок в `dashboardNode.js`/`boardManager.js`. Если источник
   не поддерживает Доску — на ноде "Дашборд" загорается error-бейдж, а
   входящая связь красится в красный (`connectionManager.setConnectionError`).
4. При каждом пересчёте графа `DashboardNode.calculate()` регистрирует
   виджет в `boardManager.registerWidget(boardId, widgetId, {...})`, где
   `widgetId` — это id самой ноды "Дашборд". Виджеты **не сериализуются** —
   это производное состояние, пересобирается заново из `calculate()`.

### Единая панель вкладок и переключение вида

Вкладки Листов и Досок отображаются в одной строке (`#layoutTabsBar`), но
это два независимых стека (`layoutManager.activeLayoutId` /
`boardManager.activeBoardId`), каждый со своей историей "какой элемент
стека активен". Чтобы клик по вкладке всегда переключал реально видимый
холст (а не только "формально активный" в своём стеке), у обоих менеджеров
есть флаг **`viewActive`** — "сейчас на экране действительно мой вид".
Переключение на один вид (`loadLayout()` / `switchToBoard()`) явно снимает
`viewActive` у соседнего менеджера через `window.layoutManager` /
`window.boardManager`, поэтому клик по вкладке, чей id совпадает с
запомненным активным, но чей вид сейчас закрыт другим стеком, всё равно
срабатывает. CSS-класс `active` на вкладке тоже требует `viewActive`, не
только совпадения id.

### Виджеты — выбор и стиль

Клик по виджету на Доске выделяет его (`.board-widget.selected`) и
открывает ту же боковую панель (`InspectorManager`), что и для нод графа —
для этого не понадобилась отдельная система, панель просто рендерит схему
полей, которую вернул `DashboardNode.getInspectorSchema()`. У ноды
"Дашборд" есть поле `widgetStyle` (`{ color, size, align }`), которое
редактируется в секции "Стиль виджета":

- **цвет акцента** — через CSS-переменную `--board-widget-accent`,
  используется числом/итоговой колонкой/шапкой таблицы;
- **размер** (Компактный/Обычный/Крупный) — `zoom` на `.board-widget-body`
  (не `transform: scale()`, чтобы соседние виджеты корректно перетекали по
  высоте на странице);
- **выравнивание** — заголовок и тело виджета.

`widgetStyle` передаётся в `boardManager.registerWidget()` **по ссылке**, а
не копией: правки из панели вызывают `boardManager.renderActiveBoard()`
напрямую и применяются мгновенно, даже если сама нода "Дашборд" сейчас
находится на неактивном Листе (где обычный пересчёт графа,
`nodeManager.calculateAll()`, её не затронет — он считает только активный
Лист). Найти нужный инстанс ноды по id виджета на любом Листе помогает
`layoutManager.findNodeAnywhere(nodeId)`.

Клик по пустому месту страницы Доски или Escape снимает выделение виджета
и закрывает панель (`boardManager.deselectWidget()` — зеркало
`nodeManager.deselectNode()` для графа). Переключение вида (Лист ⇄ Доска,
Доска ⇄ Доска) тоже сбрасывает выбор и закрывает панель предыдущего вида.
